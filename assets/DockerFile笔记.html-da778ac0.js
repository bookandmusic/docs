import{_ as l}from"./plugin-vue_export-helper-c27b6911.js";import{r as i,o,c as t,a as n,b as s,d as c,e}from"./app-5fd09ffe.js";const r="/assets/24-20230617203318-sx7n658-74a7a783.png",d="/assets/25-20230617203318-kck08g8-de4c177e.png",p="/assets/26-20230617203318-609hkzg-a4f28acc.png",u="/assets/28-20230617203318-5yyl471-207c66d0.png",v={},k=e('<h1 id="dockerfile" tabindex="-1"><a class="header-anchor" href="#dockerfile" aria-hidden="true">#</a> DockerFile</h1><h4 id="dockerfile是什么" tabindex="-1"><a class="header-anchor" href="#dockerfile是什么" aria-hidden="true">#</a> DockerFile是什么</h4><p>DockerFile是用来构建Docker镜像的文本文件，是有一条条构建镜像所需的指令和参数构成的脚本。</p><p>​<img src="'+r+'" alt="24" loading="lazy">​</p>',4),m={href:"https://docs.docker.com/engine/reference/builder/",target:"_blank",rel:"noopener noreferrer"},b=e('<p>构建三步骤</p><ol><li>编写DockerFile文件</li><li>docker build命令构建镜像</li><li>docker run 依镜像运行容器实例</li></ol><h4 id="dockerfile构建过程解析" tabindex="-1"><a class="header-anchor" href="#dockerfile构建过程解析" aria-hidden="true">#</a> DockerFile构建过程解析</h4><p><strong>DockerFile内容基础知识</strong></p><ol><li>每条保留字指令都必须为大写字母且后面跟随至少一个参数</li><li>指令按照从上到下，顺序执行</li><li>​<code>#</code>​表示注释</li><li>每条指令都会创建一个新的镜像层并对镜像进行提交。</li></ol><p><strong>Docker执行DockerFile的大致流程</strong></p><ol><li>docker从技术镜像运行一个容器</li><li>执行一条指令比鞥对容器做出修改</li><li>执行类似docker commit 的操作提交一个新的镜像层</li><li>docker 在基于刚提交的镜像运行一个新容器</li><li>执行dockerfile中的下一条指令直到所有执行执行完成。</li></ol><blockquote><p>总结</p></blockquote><p>从应用软件的角度来看，Dockerfile、Docker镜像与Docker容器分别代表软件的三个不同阶段，</p><ul><li><p>Dockerfile是软件的原材料</p></li><li><p>Docker镜像是软件的交付品</p></li><li><p>Docker容器则可以认为是软件镜像的运行态，也即依照镜像运行的容器实例</p></li></ul><p>Dockerfile面向开发，Docker镜像成为交付标准，Docker容器则涉及部署与运维，三者缺一不可，合力充当Docker体系的基石。</p><p>​<img src="'+d+`" alt="25" loading="lazy">​</p><ol><li>Dockerfile，需要定义一个Dockerfile，Dockerfile定义了进程需要的一切东西。Dockerfile涉及的内容包括执行代码或者是文件、环境变量、依赖包、运行时环境、动态链接库、操作系统的发行版、服务进程和内核进程(当应用进程需要和系统服务和内核进程打交道，这时需要考虑如何设计namespace的权限控制)等等;</li><li>Docker镜像，在用Dockerfile定义一个文件之后，docker build时会产生一个Docker镜像，当运行 Docker镜像时会真正开始提供服务;</li><li>Docker容器，容器是直接提供服务的。</li></ol><h4 id="常用保留字指令" tabindex="-1"><a class="header-anchor" href="#常用保留字指令" aria-hidden="true">#</a> 常用保留字指令</h4><blockquote><p>From</p></blockquote><p>基础镜像，当前新镜像是基于哪个镜像的，指定一个已经存在的镜像作为模板，第一条必须是from</p><blockquote><p>MANINTAINER</p></blockquote><p>镜像维护者的姓名和邮箱地址</p><blockquote><p>Run</p></blockquote><p>容器构建时需要运行的命令</p><p>两种格式：</p><p>shell格式</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&lt;</span>命令行命令<span class="token operator">&gt;</span>等同于，在终端操作的shell命令
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>‍</p><h4 id="最小镜像" tabindex="-1"><a class="header-anchor" href="#最小镜像" aria-hidden="true">#</a> 最小镜像</h4><p>​<code>scratch</code>​</p><p>官方说明：该镜像是一个空的镜像，可以用于构建基础镜像（例如 Debian、Busybox）或超小镜像，可以说是真正的从零开始构建属于自己的镜像。</p><p>从 Docker 1.5.0 开始，<code>FROM scratch</code>​在DockerFile中是一个无操作，并且不会在镜像中创建额外的层（因此以前的 2 层镜像将改为 1 层镜像）。</p><blockquote><p>解决了先有鸡还是先有蛋的问题，即第一个镜像是怎么来的</p></blockquote><p>实际上，我们可以创建自己的 scratch 镜像：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">tar</span> cv --files-from /dev/null <span class="token operator">|</span> <span class="token function">docker</span> <span class="token function">import</span> - scratch
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这样，我们就将编译并创建最终镜像集成到一个 dockerfile 里面了，而且构建出来的镜像体积也非常小。</p><p>‍</p>`,94);function g(h,f){const a=i("ExternalLinkIcon");return o(),t("div",null,[k,n("p",null,[s("官网："),n("a",m,[s("https://docs.docker.com/engine/reference/builder/"),c(a)])]),b])}const R=l(v,[["render",g],["__file","DockerFile笔记.html.vue"]]);export{R as default};
